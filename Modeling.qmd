...
new_data <- tibble(
  bmi = 50,
  smoker = factor("has smoked 5 packs", levels = c("has not smoked 5 packs", "has smoked 5 packs")),
...
final_pred <- predict(best_model, new_data, type = "class")
final_pred
...
```{r}
test_mod <- decision_tree(tree_depth = 11,
                          min_n = 20,
...
test_pred <- predict(test_model, dia_data, type = "class")

cm_df <- tibble(dia_binary = dia_data$dia_binary,
                pred_class = as.factor(test_pred$.pred_class))
